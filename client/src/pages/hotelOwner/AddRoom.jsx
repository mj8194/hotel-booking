  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Failed to add room", { id: toastId });
      }
    } catch (error) {
      console.error("Room Upload Error:", error);
      toast.error(
        error.response?.data?.message || "Error uploading room",
        { id: toastId }
      );
    } finally {
      setLoading(false);
    ...
    <form onSubmit={onSubmitHandler} className="pb-20">
        ...
        {images.map((img, index) => (
          ...
          >
            <img
              ...
              alt="upload"
              className="h-24 w-24 object-cover border rounded-md opacity-80 hover:opacity-100"
            />
            <input
              type="file"
              accept="image/*"
      ...
      <div className="w-full flex flex-wrap gap-6 mt-6">
        ...
        <div>
          ...
          <input
            type="number"
            placeholder="0"
            className="border border-gray-300 rounded p-2 w-32 outline-none"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-3 text-gray-600 max-w-lg">
        {Object.keys(inputs.amenities).map((amenity) => (
          <label
            key={amenity}
            className="flex items-center gap-2 text-sm bg-gray-50 p-2 rounded border cursor-pointer"
          >
            <input
              type="checkbox"
      ...
      <button
        type="submit"
        disabled={loading}
        className="bg-primary text-white px-10 py-3 rounded-lg mt-10 disabled:bg-gray-400"
      >
